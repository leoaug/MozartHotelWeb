...
function getItemEstoque(elemento) {
	...
	var idCentroCusto = parent.document.forms[0].idCentroCusto.value;
...
function selecionarItemEstoque(
	...
	var idCentroCusto = parent.document.forms[0].idCentroCusto.value;
	if(idCentroCusto != ''){
		calcularValorUnitarioItemEstoque(elemento, idEntidade, idCentroCusto);
	}
	else{
		parent.alerta("O campo 'Centro de Custo' deve ser preenchido antes do Item.");
		return false;
...
function calcularValorUnitarioItemEstoque(elemento, idItem, idCentroCusto) {
	url = 'app/ajax/ajax!obterValorUnitarioDevolucaoMovimentoEstoque?idItem=' + idItem 
...
function excluirLancamento(idx){
	if (confirm('Confirma a exclusão do lançamento?')){
	    document.forms[0].action = '<s:url namespace="/app/estoque" action="includeDevolucao" method="excluirLancamento" />';
	    $('#indice').val( idx );
	    parent.loading();
...
<body>
	<div class="divGrupo"
		...
		<s:form namespace="/app/estoque" action="includeDevolucao!incluirLancamentoSaida" theme="simple">
