...
<script>
    ...
    function novaReserva(){
        ...
        <s:else>
        	vForm.action = '<s:url action="manter!preparaManter.action" namespace="/app/reserva" />';
        </s:else>
        submitForm(vForm);
    }
    ...
    function novoBloqueio(){
		...
		<s:if test="%{origemCrs}">
	        vForm.action = '<s:url action="manter!preparaManterCRSBloqueio.action" namespace="/app/reserva" />';
        </s:if>
        <s:else>
        	vForm.action = '<s:url action="manter!preparaManterBloqueio.action" namespace="/app/reserva" />';
        </s:else>
        submitForm(vForm);
    }
    
    function efetuarPesquisa() {        
        document.forms[0].action = '<s:url action="pesquisar!pesquisarReservas.action" namespace="/app/reserva" />';        
        submitForm(document.forms[0]);
    }

    function relatorio() {        
        document.forms[0].action = '<s:url action="relatorio!prepararRelatorio.action" namespace="/app/reserva" />';        
        submitForm(document.forms[0]);
    }
    function gestaoBloqueio() {        
        document.forms[0].action = '<s:url action="relatorio!prepararGestaoBloqueio.action" namespace="/app/reserva" />';        
        submitForm(document.forms[0]);
    }
    ...
    function alterarReserva(){
        document.forms[0].action = '<s:url namespace="/app/reserva" action="manter" method="alterarReserva" />';        
        submitForm(document.forms[0]);
    }
    ...
    function excluirReserva(){
    	...
    	if (confirm('Deseja cancelar a reserva '+ document.getElementById('idReserva').value.split(',')[0] + ' e todos os seus apartamentos?')){
            vForm = document.forms[0];
            vForm.action = '<s:url action="pesquisar!excluirReserva.action" namespace="/app/reserva" />';
            submitForm(vForm);
        }
     }
	...
	function confirmarReserva(){
        ...
        vForm = document.forms[0];
        vForm.action = '<s:url action="pesquisar!confirmarReserva.action" namespace="/app/reserva" />';
        submitForm(vForm);
	}
	...
	function destravarReserva(){
        ...
        vForm = document.forms[0];
        vForm.action = '<s:url action="pesquisar!destravarReserva.action" namespace="/app/reserva" />';
        submitForm(vForm);
    ...
    function chart(){
         vForm = document.forms[0];
         vForm.action = '<s:url action="manterMapa!prepararPesquisaMapa.action" namespace="/app/reserva" />';
         submitForm(vForm);
     }
    ...
    function enviarReservaPorEmail(){
        vForm = document.forms[0];
        vForm.action = '<s:url action="enviarEmail!prepararEnviarReservaPorEmail.action" namespace="/app/reserva" />';
        submitForm(vForm);
    }

    function enviarReservaApartamentoPorEmail(){
        vForm = document.forms[0];
        vForm.action = '<s:url action="enviarEmail!prepararEnviarReservaApartamentoPorEmail.action" namespace="/app/reserva" />';
        submitForm(vForm);
    }
...
drawMenus(); 
 ...
 <s:form action="pesquisar!pesquisar.action" namespace="/app/reserva" theme="simple" >    
    ...
    <div class="divFiltroPaiTop">Pesquisa de Reserva</div>    
    <div id="divFiltroPai" class="divFiltroPai">
        <div id="divFiltro" class="divFiltro"  >
            <duques:filtro tableName="RESERVA_WEB" titulo="" />
        </div>
    </div>
    ...
    <div id="divMeio" class="divMeio">
        <div id="divOutros" class="divOutros" style="width: 35%;">
               <ul style="width: 90%; padding:0px; margin: 0px; list-style: none; font-size: 8pt; ">
				...
				<li style="font-size: 8pt; float:left; padding-right:5px; "><img src="imagens/imgLegVermelho.png" width="10px" height="10px" />&nbsp;Não confirmada</li>
				<li style="font-size: 8pt; float:left; padding-right:5px;"><img src="imagens/imgLegAzulClaro.png" width="10px" height="10px" />&nbsp;Com check-in</li>
				<li style="font-size: 8pt; float:left; padding-right:5px;"><img src="imagens/imgLegCinza.png" width="10px" height="10px" />&nbsp;Cancelada</li>
				<li style="font-size: 8pt; float:left; padding-right:5px;"><img src="imagens/imgLegOlive.png" width="10px" height="10px" />&nbsp;Com check-out</li>
		</ul>
        </div>
        ...
        <div id="divBotaoReserva" class="divBotao">
            ...
            <duques:botao label="Novo" imagem="imagens/iconic/png/plus-3x.png" onClick="novaReserva();" />
            <duques:botao label="Chart" imagem="imagens/iconic/png/imgChartReserva-4x.png" onClick="chart();" />
            <s:if test="%{!origemCrs || #session.CRS_PROPRIA.equals(\"TRUE\")}">
    ...
    <duques:grid titulo="Reserva" 
    			...
    			urlRetorno="pages/modulo/reserva/pesquisa.jsp">
        ...
        <duques:column labelProperty="Adm. Canal" propertyValue="gracEmpresaGds"  orderBy="gracEmpresaGds" style="text-align:left;width:120px"/>                
        <duques:column labelProperty="Total Tarifa" propertyValue="gracTotalTarifa"  orderBy="gracTotalTarifa" style="text-align:right;width:120px"/>                
    </duques:grid>
