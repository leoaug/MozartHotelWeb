...
function adicionarHospede(nomeHospede){
    document.forms[0].action = '<s:url namespace="/app/reserva" action="include" method="adicionarHospede" />';
...
function excluirHospede(idxResApto, idxHospede ){
    document.forms[0].action = '<s:url namespace="/app/reserva" action="include" method="excluirHospede" />';
...
function excluirResApto(idxResApto, corTipo){    
    if (corTipo == 'Olive' || corTipo == 'Aqua')
        ...
        if (window.confirm('- Deseja excluir todos apartamentos sem checkin?')) {
            document.forms[0].action = '<s:url namespace="/app/reserva" action="include" method="excluirReservaApto" />';
...
function atualizar() {
	...
	document.forms[0].action = '<s:url value="app/reserva/include!prepararReservaApto.action"/>';
	document.forms[0].submit();
}
