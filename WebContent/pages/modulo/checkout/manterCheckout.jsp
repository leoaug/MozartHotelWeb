...
<script type="text/javascript">
	...
	function imprimirCupomFiscal(){
        ...
        vForm.action = '<s:url action="checkout!prepararCupomFiscal.action" namespace="/app/caixa" />';
        submitForm( vForm );
	}
		...
		<s:if test="%{#session.HOTEL_SESSION.cupomfiscal == \"S\"}" >
			try{
				...
				if (numCupom == '' || numCupom == null){
					...
					vForm = document.forms[0];
			        vForm.action = '<s:url action="checkout!cancelarCupomFiscal.action" namespace="/app/caixa" />';
			        submitForm( vForm );
				}
				<s:iterator value="#session.movimentoNotaHospedagem" var="mov">
				<s:if test="%{#mov.tipoLancamentoEJB.notaFiscal == \"S\" && valorLancamento >= 0}">
	...
	function unificarTaxaCheckin(){
		...
		vForm = document.forms[0];
        vForm.action = '<s:url action="checkout!unificarTaxas.action" namespace="/app/caixa" />';
        submitForm( vForm );
    ...
    function gravarMiniPDV(){
        ...
        vForm.action = '<s:url action="checkout!gravarMiniPDV.action" namespace="/app/caixa" />';
	...
	function incluirMiniPDV(){
        ...
        vForm.action = '<s:url action="checkout!incluirMiniPDV.action" namespace="/app/caixa" />';
	...
	function excluirMiniPDV( idx ) {
        ...
        if (confirm('Deseja realmente excluir esse lançamento?')){        
            $('#id').val(idx);
        	vForm.action = '<s:url action="checkout!excluirMiniPDV.action" namespace="/app/caixa" />';
        	submitForm( vForm );
        }
	}
    ...
    function cancelarMiniPDV(){
        vForm = document.forms[0];
        vForm.action = '<s:url action="checkout!cancelarMiniPDV.action" namespace="/app/caixa" />';
        submitForm( vForm );
    }
    ...
    function liberarHospede( idx ){
        ...
        if (confirm('Deseja realmente liberar este hóspede?')){        
            $('#idxCliente').val(idx);
            vForm.action = '<s:url action="checkout!liberarHospede.action" namespace="/app/caixa" />';
            submitForm( vForm );
        }
    }    
    ...
    function liberarCheckin(){
        ...
        vForm.action = '<s:url action="checkout!liberarApartamento.action" namespace="/app/caixa" />';
        submitForm( vForm );
    }
    ...
    function incluirMovimentoDevolucao( idxDevolucao ){
        ...
        vForm.action = '<s:url action="checkout!incluirMovimentoDevolucao.action" namespace="/app/caixa" />';
        killModal();
        submitForm( vForm );
    }
    function incluirDevolucao(){
        ...
        vForm.idMovimentoObjeto.value= valor;
        vForm.action = '<s:url action="checkout!devolverObjetos.action" namespace="/app/caixa" />';
        submitForm( vForm );
    }
    ...
    function gravarCancelamentoNota(){
        ...
        vForm.action = '<s:url action="checkout!cancelarNotaHospedagem.action" namespace="/app/caixa" />';
        submitForm( vForm );
    ...
    function gerarNotaFiscal(){
        ...
        vForm.action = '<s:url action="checkout!prepararNotaFiscal.action" namespace="/app/caixa" />';
        submitForm( vForm );
    ...
    function imprimeNotaHospedagem(){
        ...
        vForm = document.forms[0];
        vForm.action = '<s:url action="checkout!prepararNotaHospedagem.action" namespace="/app/caixa" />';
        submitForm( vForm );
    }
    ...
    function gravarAlteracoes(){
        ...
        vForm = document.forms[0];
        vForm.action = '<s:url action="checkout!gravarSincronizacao.action" namespace="/app/caixa" />';
        submitForm( vForm );
    }
    ...
    function gravarCheckout(){
        ...
        if ( $('#valorPagamento').val() != $('#valorAdicionado').val() ){
            alerta('Não pode restar diferença para realizar o check-out.');
            return false;
        }
    
        vForm = document.forms[0];
        vForm.action = '<s:url action="checkout!gravarCheckout.action" namespace="/app/caixa" />';
        submitForm( vForm );
    }
    ...
    function realizarCheckout(idxCliente){
        ...
        $('#idxCliente').val(idxCliente);
        vForm.action = '<s:url action="checkout!prepararPopCheckout.action" namespace="/app/caixa" />';
        submitForm( vForm );
    }
    ...
    function imprimirExtrato(idxCliente){
        vForm = document.forms[0];
        $('#idxCliente').val(idxCliente);
        vForm.action = '<s:url action="checkout!imprimirExtrato.action" namespace="/app/caixa" />';
        submitForm( vForm );
    }
    ...
    function excluirPagamento(idxPagamento){
        if (confirm('Confirma a exclusão do pagamento?')){
            vForm = document.forms[0];
            $('#id').val(idxPagamento);
            vForm.action = '<s:url action="checkout!excluirPagamentoCheckout.action" namespace="/app/caixa" />';
            submitForm( vForm );
        }
    }
    ...
    function incluirPagamento(){
        ...
        if (texto == 'VISA WEB' || texto == 'AMEX WEB' || texto == 'CREDICARD WEB' || texto == 'DINERS WEB' || texto == 'MASTERCARD WEB'){
            ...
            if (confirm('Esta operação realizará uma transação on-line no cartão do cliente. Confirma?')){
		        ...
		        vForm.action = '<s:url action="checkout!adicionarPagamentoCheckout.action" namespace="/app/caixa" />';
		        submitForm( vForm );
            }
    ...
    function incluirMovimento(){
        ...
        vForm.action = '<s:url action="checkout!incluirMovimento.action" namespace="/app/caixa" />';
        submitForm( vForm );
    }
    ...
    function caixaGeral(){
            vForm = document.forms[0];
            vForm.action = '<s:url action="caixaGeral!pesquisar.action" namespace="/app/caixa" />';
...
<div class="divCadastroBotoes" style="width: 98%;">
	...
	<duques:botao label="Substituir hóspede" style="width:170px;" imagem="imagens/imgSubstituirHospede.png" onClick="substituirHospedeNovo()" />
</div>
...
<s:form action="checkout.action" theme="simple" namespace="/app/caixa">
	...
	<div class="divFiltroPaiTop" style="width: 220px;">Check-out</div>
	...
	<div class="divLinhaCadastro">
		<div class="divItemGrupo">
		<p style="width: 100px;">Apartamento:</p>
		<b><font color="red"><s:property
			value="#session.checkinCorrente.apartamentoEJB" /> </font></b>
		</div>
	</div>
	...
	<ul>
		...
		<img src="imagens/iconic/png/print-3x.png" title="Extrato parcial"
			...
			<img src="imagens/iconic/png/imgCarrinho-3x.png" title="Mini PDV"
...
<s:if test='%{#request.abrirPopupExtrato == "true"}'>
	...
	<s:if test='%{idxCliente == "0"}'>
		...
		Apto: <font color="red"><s:property
			value="#session.checkinCorrente.apartamentoEJB" /> </font>&nbsp;&nbsp;&nbsp;Nome:
		<s:property
		...
		<s:iterator value="#session.checkinCorrente.roomListEJBList"
			...
			<s:if test="%{idxCliente == #row.index + 1}">
				<div id="divExtrato"
					style="display: none; background-color: white; border: 1px solid rgb(0, 82, 255); position: absolute; overflow: auto; width: 800px; height: 450px;">
				<div
					style="float: left; text-align: left; height: 25px; color: white; font-family: verdana; background-color: rgb(0, 173, 255); width: 700px; font-size: 14px; font-weight: bold;">
				<img src="imagens/iconMozart.png" />Extrato Parcial</div>
				<div
					style="float: left; width: 70px; height: 25px; font-family: verdana; background-color: rgb(0, 173, 255);">
				<img height="24px;" width="24px" src="imagens/iconic/png/print-3x.png"
				...
				<div
					style="float: left; font-size: 14px; width: 740px; text-align: left;">
				Apto: <font color="red"><s:property
					value="#session.checkinCorrente.apartamentoEJB" />
...
<div
	style="float: left; text-align: left; height: 25px; color: white; width: 645px; font-size: 14px; font-weight: bold; font-family: verdana; background-color: rgb(0, 173, 255);">
Check-out</div>
<div
...
<s:if test="%{valorPagamento.compareTo(valorAdicionado) == 0 && #session.movimentoPagamentoList.size() > 0}">
	...
	<s:if test="%{liberaGravacao || (#session.HOTEL_SESSION.cupomfiscal == \"S\" && #session.notaHospedagem.tipoNotaFiscal == \"CF\")}">
		<img align="right" height="24px;" width="24px"
			src="imagens/iconic/png/checkVerde-3x.png" title="Confirmar"
			onclick="gravarCheckout();" />
	</s:if>
	<s:else>
		<img align="right" height="24px;" width="24px"
			src="imagens/iconic/png/check-3x.png" style="cursor: auto;"
			title="Confirmar: Imprima a nota de hospedagem ou fiscal ou cupom fiscal"
			 />
	</s:else>
	...
	<img align="right" height="24px;" width="24px"
		src="imagens/iconic/png/check-3x.png" title="Confirmar"
		style="cursor: auto;" />
...
<p style="width: 80px;">Apto:</p>
<font color="red"><s:property
	value="#session.checkinCorrente.apartamentoEJB" /> </font></div>
